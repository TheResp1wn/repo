...
- (void)followTwitter {
    openTwitter([NSURL URLWithString:@"https://twitter.com/resp1wn"]);
}


- (void)donate {
    openURL([NSURL URLWithString:@"https://paypal.me/duartrizani"]);
}

- (void)sendEmail {
    openURL([NSURL URLWithString:@"mailto:resp1wn.res@gmail.com?subject=2TapLock"]);
}
...
- (id)initWithSpecifier:(PSSpecifier *)specifier {
    ...
    if (self) {
        ...
        [_headerLabel setText:@"2TapLock"];
        [_headerLabel setFont:font];

        _subheaderLabel = [[UILabel alloc] init];
        [_subheaderLabel setText:@"by Resp1wn"];
